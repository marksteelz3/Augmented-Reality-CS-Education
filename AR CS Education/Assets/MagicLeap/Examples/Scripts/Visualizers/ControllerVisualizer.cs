...
namespace MagicLeap
{
    ...
    public class ControllerVisualizer : MonoBehaviour
    {
        ...
        public GameObject objectToBeReleased;
        public GameObject freeObjects;
        public Collider parentClass;
        public Collider subClassPart;
        ...
        private void HandleOnButtonUp(byte controllerId, MLInputControllerButton button)
        {
            ...
            if (controller != null && controller.Id == controllerId)
            {
                if (button == MLInputControllerButton.Bumper)
                {
                    ...
                    //MARK's CODE:
                    Grabber colliderScript = controllerCollider.GetComponent<Grabber>();

                    if (colliderScript.objectHeld != null && StepTracker.instance.objectGrabbed == false){
                        if (colliderScript.objectHeld.tag == "PositionSetter" || colliderScript.objectHeld.tag == "NextStepTest")
                        {
                            colliderScript.objectHeld.GetComponent<Rigidbody>().useGravity = false;
                            colliderScript.objectHeld.transform.SetParent(this.gameObject.transform);
                            objectToBeReleased = colliderScript.objectHeld;
                            StepTracker.instance.objectGrabbed = true;
                        }
                        if (colliderScript.objectHeld.tag == "SetPosition")
                        {
                            StepTracker.instance.click("setPosition");
                        }
                        if (colliderScript.objectHeld.tag == "AdjustFloor")
                        {
                            StepTracker.instance.click("lowerFloor");
                        }
                        if (StepTracker.instance.step == 5 && colliderScript.objectHeld.tag == "Starter")
                        {
                            //Step 6
                        ...
                        else if (StepTracker.instance.step == 6 && colliderScript.objectHeld.tag == "Handle")
                        {
                            //Step 7
                            GameObject classClone = Instantiate(colliderScript.objectHeld, colliderScript.objectHeld.transform.position, colliderScript.objectHeld.transform.rotation);
                            classClone.transform.SetParent(this.gameObject.transform);
                            objectToBeReleased = classClone;
                            StepTracker.instance.objectGrabbed = true;
                            StepTracker.instance.currAud = StepTracker.instance.step + 3;
                            StepTracker.instance.step++;
                            StepTracker.instance.click("nextPart");
                        }
                        else if (StepTracker.instance.step == 8 && colliderScript.objectHeld.tag == "HammerHead")
                        {
                            //Step 8.5
                            GameObject classClone = Instantiate(colliderScript.objectHeld, colliderScript.objectHeld.transform.position, colliderScript.objectHeld.transform.rotation);
                            classClone.transform.SetParent(this.gameObject.transform);
                        ...
                        else if (StepTracker.instance.step == 9 && colliderScript.objectHeld.tag == "Handle")
                        {
                            //Step 9.5
                            GameObject classClone = Instantiate(colliderScript.objectHeld, colliderScript.objectHeld.transform.position, colliderScript.objectHeld.transform.rotation);
                            classClone.transform.SetParent(this.gameObject.transform);
                        ...
                        else if (StepTracker.instance.step == 10 && colliderScript.objectHeld.tag == "KnifeBlade")
                        {
                            //Step 10.5
                            GameObject classClone = Instantiate(colliderScript.objectHeld, colliderScript.objectHeld.transform.position, colliderScript.objectHeld.transform.rotation);
                            classClone.transform.SetParent(this.gameObject.transform);
                        ...
                        else if (StepTracker.instance.step == 11 && colliderScript.objectHeld.tag == "Knife")
                        {
                            //Step 11.5
                            GameObject classClone = Instantiate(colliderScript.objectHeld, colliderScript.objectHeld.transform.position, colliderScript.objectHeld.transform.rotation);
                            classClone.transform.SetParent(this.gameObject.transform);
                    ...
                    else if (colliderScript.objectHeld != null && StepTracker.instance.objectGrabbed == true)
                    {
                        objectToBeReleased.transform.SetParent(freeObjects.transform);
                        //objectToBeReleased.GetComponent<Rigidbody>().useGravity = true;
                        if (objectToBeReleased.tag == "PositionSetter" || objectToBeReleased.tag == "NextStepTest"){
                            objectToBeReleased.GetComponent<Rigidbody>().useGravity = true;
                            objectToBeReleased.GetComponent<BoxCollider>().isTrigger = false;
                        }
                        if (StepTracker.instance.step == 7 && objectToBeReleased.tag == "Handle")
                        {
                            //Step 8
                            objectToBeReleased.GetComponent<Rigidbody>().useGravity = true;
                            objectToBeReleased.GetComponent<BoxCollider>().isTrigger = false;
                        ...
                        else if (StepTracker.instance.step == 9 && objectToBeReleased.tag == "Handle")
                        {
                            //Step 10
                            objectToBeReleased.GetComponent<Rigidbody>().useGravity = true;
                            objectToBeReleased.GetComponent<BoxCollider>().isTrigger = false;
